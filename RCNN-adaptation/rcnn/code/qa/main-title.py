...
from nn import EmbeddingLayer, LSTM, GRU, RCNN, Dropout, apply_dropout, CNN
import myio
...
class Model:
    ...
    def ready(self):
        ...
        diff = neg_scores - pos_scores + self.args.margin
    ...
    def train(self, ids_corpus, train, dev=None, test=None):
        ...
        for epoch in xrange(max_epoch):
            ...
            train = myio.read_annotations(args.train, training_data_percent = args.training_data_percent)
            train_batches = myio.create_batches(ids_corpus, train, batch_size,
                                    padding_id, pad_left = not args.average, include_generated_questions=True)
            N =len(train_batches)
            ...
            for i in xrange(N):
                ...
                cur_cost, cur_loss, grad_norm = train_func(idts, idps)
                train_loss += cur_loss
                train_cost += cur_cost
                ...
                if i == N-1:
                    ...
                    if dev_MRR > best_dev:
                        ...
                        result_table.add_row(
                            [ epoch ] +
                            [ "%.2f" % x for x in [ dev_MAP, dev_MRR, dev_P1, dev_P5 ] + [0.0, 0.0, 0.0, 0.0] ]
    ...
    def just_eval(self, dev=None, test=None):
        self.dropout.set_value(0.0)
        eval_func = theano.function(
                inputs = [ self.idts ],
                outputs = self.scores,
        ...
        if dev is not None:
            dev_MAP, dev_MRR, dev_P1, dev_P5 = self.evaluate(dev, eval_func)
            print("{}\t{}\t{}\t{}\t".format(dev_MAP, dev_MRR, dev_P1, dev_P5)),
        if test is not None:
            test_MAP, test_MRR, test_P1, test_P5 = self.evaluate(test, eval_func)
            print("{}\t{}\t{}\t{}\n".format(test_MAP, test_MRR, test_P1, test_P5))
        return [dev_MAP, dev_MRR, dev_P1, dev_P5]
        # return [dev_MAP, dev_MRR, dev_P1, dev_P5, test_MAP, test_MRR, test_P1, test_P5]
    ...
    def evaluate(self, data, eval_func):
        ...
        for idts, idbs, labels, weights in data:
            ...
            for i, qq_query_weight in enumerate(qq_query_weights):
                ...
                idts_individual = np.array([idts_t[i]] + idts_t[len(qq_query_weights):].tolist(), dtype=np.int32).transpose()
                ...
                for j, cq_weights in enumerate(weights[1:]):
                    ...
                    individual_scores[j] += scores_for_qq[:len(cq_weights)].tolist()
                    individual_scores_weights[j] += [cq_weight * qq_query_weight for cq_weight in cq_weights]
                    scores_for_qq = scores_for_qq[len(cq_weights):]

            # now we determine the weights
            scores = []
            for individual_scores_item, individual_scores_weights_item in zip(individual_scores, individual_scores_weights):
                scores.append(np.average(individual_scores_item, weights=individual_scores_weights_item))
    ...
    def analyze(self, file_name, embedding_layer, data):
        self.dropout.set_value(0.0)
        eval_func = theano.function(
            # inputs = [ self.idts, self.idbs ],
            inputs = [ self.idts],
        ...
        for idts, idbs, labels in data:
            ...
            candidates = {ii:(' '.join(embedding_layer.map_to_words(i))).replace('<padding>','').strip() for ii,i in enumerate(np.transpose(idts).tolist())}
            candidate_print = ["{} : {} : {}".format(rl,sc,candidates[i+1]) for i,rl,sc in zip(ranks.tolist(), ranked_labels.tolist(), ranked_scores.tolist())]
            st = '\n'.join(["Q> "+candidates[0]]+candidate_print)
            fw.write('{}\n\n'.format(st))
...
def main(args):
    raw_corpus = myio.read_corpus(args.corpus, args.translations or None, args.translatable_ids or None, args.generated_questions_train or None)

    generated_questions_eval = myio.read_generated_questions(args.generated_questions)
    ...
    ids_corpus = myio.map_corpus(raw_corpus, embedding_layer, max_len=args.max_seq_len, generated_questions=generated_questions_eval)
    ...
    if args.dev:
        ...
        dev = myio.read_annotations(args.dev, K_neg=args.dev_pool_size, prune_pos_cnt=-1)
    ...
    if args.train:
        start_time = time.time()
        train = myio.read_annotations(args.train, training_data_percent = args.training_data_percent)
        train_batches = myio.create_batches(ids_corpus, train, args.batch_size, padding_id, pad_left = not args.average, include_generated_questions=True)
        ...
        if len(args.analyze_file.strip())>0:
            model.load_pretrained_parameters(args)
            file_name =  args.analyze_file.strip() # 'AskUbuntu.Rcnn_analysis3.gt(es)-gt.txt'
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--translations",
            type = str,
            default = ""
        )
    argparser.add_argument("--translatable_ids",
            type = str,
            default = ""
        )
    ...
    argparser.add_argument("--do_train",
            type = int,
            default = 0
        )
    argparser.add_argument("--do_evaluate",
            type = int,
            default = 0
        )    
    argparser.add_argument("--analyze_file",
            type = str,
            default = ""
        )
    argparser.add_argument("--trainable_embeddings",
            type = int,
            default = 0
        )
    argparser.add_argument("--dev_pool_size",
            type = int,
            default = -1
        )
    argparser.add_argument("--training_data_percent",
            type = int,
            default = 100
        )
    argparser.add_argument("--generated_questions",
            type = str,
            default = ""
        )
    argparser.add_argument("--generated_questions_train",
            type = str,
            default = ""
        )
